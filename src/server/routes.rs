...
pub fn create_router(state: ServerState) -> Router {
    ...
    Router::new()
        ...
        .route("/api/nav/raw-acquisitions", post(nav::nav_raw_acquisition_upload))
        .route("/api/nav/initial-observations", post(nav::nav_initial_observations))
        .route("/api/nav/setup-registrations-3d", post(nav::nav_setup_registration_3d))
        .route("/api/nav/setup-registrations-3d/:id", get(nav::nav_setup_registration_3d_status))
        ...
        .route("/api/nav/navigation-sessions/:id/completion", post(nav::nav_finish_navigation_session))
        .route("/api/gpu/test", post(handlers::gpu_test))
        .route("/api/gpu/volume/test", post(handlers::gpu_volume_test))
