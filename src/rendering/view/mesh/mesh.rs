...
impl Measurement {
    ...
    pub fn clear(&mut self) {
        self.start = Vec3::new(0.0, 0.0, 0.0);
        self.end = Vec3::new(0.0, 0.0, 0.0);
    }

    pub fn set_start(&mut self, p: Vec3) {
        self.start = p;
    }

    pub fn set_end(&mut self, p: Vec3) {
        self.end = p;
    }

    pub fn set_point(&mut self, start: bool, p: Vec3) {
        if start {
            self.set_end(p);
        } else {
            self.set_start(p);
    ...
    pub fn distance_mm(&self, dims:[f32;3], spacing:[f32;3])->Option<f32>{
        let p1 = self.start;
        let p2 = self.end;
        let dx = (p2.x-p1.x) *dims[0] *spacing[0];
        let dy = (p2.y-p1.y) *dims[1] *spacing[1];
        let dz = (p2.z-p1.z) *dims[2] *spacing[2];
        Some((dx*dx+dy*dy+dz*dz).sqrt())
    }
}

pub struct CpuVolumeSampler {
    volume: Vec<u16>,
    width: usize,
    height: usize,
    depth: usize,
...
impl CpuVolumeSampler {
    pub fn sample(&self, x: f32, y: f32, z: f32) -> f32 {
        let ix = x.clamp(0.0, 1.0) * (self.width as f32 - 1.0);
        let iy = y.clamp(0.0, 1.0) * (self.height as f32 - 1.0);
        let iz = z.clamp(0.0, 1.0) * (self.depth as f32 - 1.0);
        ...
        let idx =
            z0 * self.width * self.height +
            y0 * self.width +
            x0;

        self.volume[idx] as f32
    }

    pub fn sample_line(&self,start: [f32;3],end: [f32;3],steps: usize) -> Vec<f32> {
...
pub struct MeasureRenderContext {
    pub pipeline: Arc<RenderPipeline>,
...
impl MeasureRenderContext {
    pub fn new(
        device:&Device,
        target_format:wgpu::TextureFormat,
    )-> Self {
        let pipeline = Arc::new(create_measure_pipeline(
            device,
            target_format,
        ));
        ...
        let vertex_buffer = device.create_buffer(
            &wgpu::BufferDescriptor {
                label: Some("MeasureVB"),
                size: (2 * std::mem::size_of::<MeasureVertex>()) as u64,
        ...
        let uniform_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("MeasureUniformBuffer"),
        ...
        let uniform_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("MeasureUniformBindGroup"),
            layout: &uniform_bind_group_layout,
            entries: &[wgpu::BindGroupEntry {
                binding: 0,
                resource: uniform_buffer.as_entire_binding(),
    ...
    pub fn update_measurement(&self, queue: &wgpu::Queue, line: &Measurement, mvp: Mat4) {
        let vertice = [
            MeasureVertex {position: line.start.into()},
            MeasureVertex {position: line.end.into()}
        ];
        queue.write_buffer(&self.vertex_buffer, 0, bytemuck::cast_slice(&vertice));
