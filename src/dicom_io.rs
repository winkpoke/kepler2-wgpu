...
mod tests {
    ...
    #[test]
    fn test_dicom_export(){
        let args = vec![
            "dicom_1", 
            "C:\\share\\head_mold", 
            "C:\\share\\image.png", 
            "--out", "C:\\share\\output.dcm",
            "--encapsulate",             // encapsulate
            "--transfer-syntax", "1.2.3",// transfer_syntax
            "--retain-implementation",   // retain_implementation
            "-v"                         // verbose
        ];
        ...
        assert_eq!(app.dcm_file, PathBuf::from("base.dcm"));
        assert_eq!(app.img_file, PathBuf::from("image.png"));
        assert_eq!(app.output, Some(PathBuf::from("output.dcm")));
        assert!(app.encapsulate);
        assert_eq!(app.transfer_syntax, Some("1.2.3".to_string()));
        assert!(app.retain_implementation);
        assert!(app.verbose);
    }
