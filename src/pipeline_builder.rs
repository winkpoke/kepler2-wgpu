...
use crate::error::{KeplerError, KeplerResult};
use crate::pipeline::{PipelineKey, PipelineManager, vertex_layout_signature, default_slice_bgl_signature};
...
impl<'a> PipelineBuilder<'a> {
    ...
    fn build_texture_quad(&mut self, req: &PipelineRequest) -> KeplerResult<Arc<wgpu::RenderPipeline>> {
        let target_format = match req.params.target_format {
            Some(fmt) => fmt,
            None => crate::pipeline::get_swapchain_format().unwrap_or(wgpu::TextureFormat::Rgba8Unorm),
        };
        ...
        let pipeline = crate::pipeline::create_texture_quad_pipeline(self.device, bgl_arr, vertex_buffers, target_format);
    ...
    fn build_mesh_basic(&mut self, _req: &PipelineRequest) -> KeplerResult<Arc<wgpu::RenderPipeline>> {
        #[cfg(feature = "mesh")]
        {
            let p = crate::pipeline::get_or_create_mesh_pipeline(self.manager, self.device);
            return Ok(p);
        }
