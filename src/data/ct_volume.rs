...
//         let new_origin_world = self.base.matrix * origin_voxel;

//         // Construct new base matrix
//         // The orientation and scaling remain the same, only the translation (last column) changes.
//         let mut new_matrix_data = self.base.matrix.to_cols_array();
//         new_matrix_data[0] = new_origin_world[0];
//         new_matrix_data[1] = new_origin_world[1];
//         new_matrix_data[2] = new_origin_world[2];

//         let new_base = Base {
//             label: format!("{}_cropped", self.base.label),
//             matrix: Mat4::from_cols_array(&new_matrix_data),
//         };
...
//         let inv_matrix = self.base.matrix.inverse();

//         // Transform all corners to voxel space to find the extent in voxel grid
//         for p in corners {
//             let v_pos = inv_matrix * Vec3::from(p);
//             for i in 0..3 {
