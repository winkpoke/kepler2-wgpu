...
impl PixelData {
    ...
    pub fn from_le_bytes(bytes: &[u8], pixel_type: PixelType) -> MedicalImagingResult<Self> {
        match pixel_type {
            PixelType::UInt8 => Ok(Self::UInt8(bytes.to_vec())),
            PixelType::UInt16 => Ok(Self::UInt16(
                bytes
                    .chunks_exact(2)
                    .map(|c| u16::from_le_bytes([c[0], c[1]]))
                    .collect(),
            )),
            PixelType::Int16 => Ok(Self::Int16(
                bytes
                    .chunks_exact(2)
                    .map(|c| i16::from_le_bytes([c[0], c[1]]))
                    .collect(),
            )),
            PixelType::Int32 => Ok(Self::Int32(
                bytes
                    .chunks_exact(4)
                    .map(|c| i32::from_le_bytes([c[0], c[1], c[2], c[3]]))
                    .collect(),
            )),
            PixelType::Float32 => Ok(Self::Float32(
                bytes
                    .chunks_exact(4)
                    .map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]]))
                    .collect(),
            )),
            PixelType::Float64 => Ok(Self::Float64(
                bytes
                    .chunks_exact(8)
        ...
        match pixel_type {
            ...
            PixelType::Float32 => {
                for chunk in raw_data.chunks_exact(4).take(voxel_count) {
                    ...
                    let val = (val - 0.2669) / 0.2669 * 1000.0;
                    let val = (val * slope + intercept).round() as i16;
                    voxel_data.push(val);
                }
            }
            other => {
                return Err(MedicalImagingError::UnsupportedPixelType {
                    pixel_type: format!("{:?}", other),
                })
            }
        };
    ...
    pub fn from_be_bytes(bytes: &[u8], pixel_type: PixelType) -> MedicalImagingResult<Self> {
        match pixel_type {
            PixelType::UInt8 => Ok(Self::UInt8(bytes.to_vec())),
            PixelType::UInt16 => Ok(Self::UInt16(
                bytes
