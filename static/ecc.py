...
def gen_projs(file_path):
    ...
    # save_mhd(os.path.join(file_path, "p1.mhd"), p)

    # p2 = np.power(p, 2)
    # save_mhd(os.path.join(file_path, "p2.mhd"), p2)

    # p3 = np.power(p, 3)
    # save_mhd(os.path.join(file_path, "p3.mhd"), p3)

    # p4 = np.power(p, 4)
    # save_mhd(os.path.join(file_path, "p4.mhd"), p4)
    return p
...
def main():
    ...
    parser = argparse.ArgumentParser(description="投影数据校正处理工具")
    parser.add_argument("-i", "--input", required=True, help="输入文件夹路径")
    parser.add_argument("-O", "--output", required=True, help="输出 .mhd 文件路径")
    args = parser.parse_args()

    file_path = args.input
    output_mhd = args.output

    print("开始处理投影数据...")
    p = gen_projs(file_path)
    # c = calc_c4()
    c = [-2.495e-03,  2.794e-01,  6.475e+00, -6.625e-01, -4.847e-02]
    # proj1 = np.fromfile(r"E:\spine_4BBs\p1_new.raw", dtype="float32")
    # proj2 = np.fromfile(r"E:\spine_4BBs\p2_new.raw", dtype="float32")
    # proj3 = np.fromfile(r"E:\spine_4BBs\p3_new.raw", dtype="float32")
    # proj4 = np.fromfile(r"E:\spine_4BBs\p4_new.raw", dtype="float32")
    ...
    # save_mhd(os.path.join(file_path, "projections_corrected_new.mhd"), proj)
    print("正在导出最终结果...")
    save_mhd(output_mhd, proj)
    print(f"处理完成！输出文件：{output_mhd}")
