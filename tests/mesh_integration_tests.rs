...
mod mesh_integration_tests {
    ...
    async fn test_performance_monitoring_integration() {
        ...
        assert!(
            quality_change.is_none(),
            "Quality should not change on first frame"
        );
    }
    ...
    async fn test_quality_adjustment() {
        ...
        for _ in 0..5 {
            ...
            if adjustment.is_some() {
                assert!(
                    (adjustment.unwrap() as u8) < (QualityLevel::High as u8),
        ...
        assert!(
            (current_quality as u8) < (QualityLevel::High as u8),
            "Quality should be reduced after slow frames"
        );
    }
            ...
            match quality {
                QualityLevel::Minimal => {
                    assert!(
                        ...
                        "Minimal quality should use wireframe"
                    );
                }
                QualityLevel::Maximum => {
                    assert!(
                        !settings.wireframe_mode,
                        "Maximum quality should not use wireframe"
                    );
                    assert!(
                        settings.lighting_enabled,
                        "Maximum quality should have lighting"
                    );
                }
                _ => {
    ...
    async fn test_performance_benchmark() {
        ...
        assert!(
            ...
            "Benchmark should take measurable time"
        );
        assert!(
            stats.current_quality != QualityLevel::Minimal || true,
